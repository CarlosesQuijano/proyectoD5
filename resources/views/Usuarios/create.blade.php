           ...
           <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
               <div class="p-6 text-gray-900">
                   <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                     <p class="flex justify-center align-center">Ingresa los datos del nuevo usuario:<p>
                        @if (session('errorValidator'))
                    <span class="bg-red-500 p-1">{{session('errorValidator')}}</span>
                    @endif
                   <form class="p-5" action="{{route("Usuarios.store")}}" method="POST">
                       @csrf
                       ...
                       <button type="submit" class="btn" style="background-color: #562a80; color:white;">Crear Usuarios</button>
                   </form>
